...
contract Ticket is IERC721 {
    ...
    mapping(uint256 => Metadata) public ticketMetadatas;
    ...
    */
    ...
    modifier isTokenOwner(uint256 _tokenId, address _owner) {
        require(ticketMetadatas[_tokenId].owner == _owner, "Not the owner of the token");
        _;
    }
    ...
    modifier checkApproval(address _from, address _to, uint256 _tokenId) {
        if(msg.sender != _from) {
            require( msg.sender == owner || operatorApprovalsForAll[ticketMetadatas[_tokenId].owner][msg.sender] == true || operatorApprovals[_tokenId] == msg.sender, "Caller is not approved to transfer this token");
        }
        _;
    }

    modifier tokenExists(uint256 _tokenId) {
        require(ticketMetadatas[_tokenId].owner != address(0), "Token does not exist");
        _;
    }
    ...
    function ownerOf(uint256 _tokenId) external view tokenExists(_tokenId) returns (address) {
        return ticketMetadatas[_tokenId].owner;
    }
    ...
    function updateTokenOwner(uint256 _tokenId, address _to) private {
        ticketMetadatas[_tokenId].previousOwner = ticketMetadatas[_tokenId].owner;
        ticketMetadatas[_tokenId].owner = _to;
        balances[ticketMetadatas[_tokenId].previousOwner ] -= 1;
        balances[_to] += 1;
    }
    ...
    function approve(address _approved, uint256 _tokenId) external tokenExists(_tokenId) {
        ...
        require (msg.sender == owner || operatorApprovalsForAll[ticketMetadatas[_tokenId].owner][msg.sender] == true, "Caller is not approved to approve this token"); 
    ...
    function mint(address _to, uint256 _concertId, uint256 _ticketPrice, string calldata _ticketURI, address _artist ) external isOwner() {
        ticketMetadatas[ticketId] = Metadata(_concertId, ticketId, _ticketPrice, _ticketURI, _to, address(0), _artist);
        balances[_to] += 1;
        ticketId += 1;
    ...
    function getAttendees(uint256 _concertId) external view returns (address[] memory) {
        ...
        for(uint256 i = 0; i < ticketId; i++) {
            if(ticketMetadatas[i].concertId == _concertId) {
                attendees[count] = ticketMetadatas[i].owner;
                count += 1;
            }
        }
        return attendees;
